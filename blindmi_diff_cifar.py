...
j=0
k=0
error_mat=[6,52,47,6,6,6,52,52,6,6,6,6,6,52,52,52,52,38,52,6,38,52,6,52,6,47,47,47,52,6,52,6,6,6,6,52,60,6,52,6,47,6,6,38,52,6,38,52,6,52,6,52,6,38,52,60,
            52,6,6,52,6,52,52,6,47,47,52,6,38,47,38,6,6,6,52,38,38,47,6,6,52,6,6,52,6,52,6,6,60,38,52,38,52,60,6,47,6,6,6,60]
while i<11999:
    x_[i]=x_train_tar[k]
    x_[i+1]=x_test_tar[(error_mat[classes])*60+j]
    m_[i+1]=0
    y_[i]=y_train_tar[k]
    y_[i+1]=y_test_tar[(error_mat[classes])*60+j]
    i+=2
    k+=1
    j+=1
    if j%60==0:
...
def diff_Mem_attack(x_, y_true, m_true, target_model, non_Mem_Generator=sobel):
    ...
    dd = np.ones((12000, 5))
    for i in range(100):
        ll = np.c_[mix[i * 120:(i + 1) * 120][:], m_true[i * 120:(i + 1) * 120]]
    ...
    writer=pd.ExcelWriter('result_of_DIFFCLASS.xlsx')
