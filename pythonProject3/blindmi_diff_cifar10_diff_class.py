...
j=0
k=0
error_mat=[6,0,0,0,0,0,0,0,0,0]
while i<17999:
    x_[i]=x_train_tar[k]
    x_[i+1]=x_test_tar[(error_mat[classes])*900+j]
    m_[i+1]=0
    y_[i]=y_train_tar[k]
    y_[i+1]=y_test_tar[(error_mat[classes])*900+j]
    i+=2
    k+=1
    j+=1
    if j%900==0:
...
def diff_Mem_attack(x_, y_true, m_true, target_model, non_Mem_Generator=sobel):
    ...
    for i in range(10):
        for j in range(10):
            diff_mat[i][j]=mmd_loss(mix[i*(1800):(i+1)*1800][:3],mix[j*1800:(j+1)*1800][:3],weight=1)
    diff_mat=pd.DataFrame(diff_mat)
    diff_mat.to_excel('cifar_diffmat.xlsx')
    ...
    dd = np.ones((18000, 5))
    for i in range(10):
        ll = np.c_[mix[i * 1800:(i + 1) * 1800][:], m_true[i * 1800:(i + 1) * 1800]]
        np.random.shuffle(ll)
        l = list(ll)
        dd[i * 1800:(i + 1) * 1800][:] = ll
    ...
    writer=pd.ExcelWriter('result_of_CIFAR10_DIFFCLASS.xlsx')
