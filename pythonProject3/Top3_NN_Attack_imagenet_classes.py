...
NUM_CLASSES = 1
LEARNING_RATE = 5e-5
NN_ATTACK_WEIGHTS_PATH = "weights/NN_Attack/BlackBox/Top3NN_Attack_{}_{}.hdf5".format(DATA_NAME, SHADOW_MODEL_GENRE)
...
c_train = np.sort(Shadow_Model.predict(np.r_[x_train_sha, x_test_sha]), axis=1)[:, ::-1][:, :3]
x_train_tar, y_train_tar, x_test_tar, y_test_tar, m_test = globals()['loads_' + DATA_NAME]('TargetModel')
...
c_test = np.sort(Target_Model.predict(x_), axis=1)[:, ::-1][:, :3]
...
def evaluate(x_test, y_test):
    ...
    for k in range(200):
        ...
        member_=sum(y_test[index:index+40])
        ...
        nonmember_ratio.append(nonmember_in_m/(40-sum(y_test[index:index+40])))
    ...
    result.to_excel('Top3_NN_Attack_classes_imagenet.xlsx')
