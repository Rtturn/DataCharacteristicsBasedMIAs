...
x_train, y_train, x_test, y_test, m_true = globals()['loads_' + DATA_NAME]()
Target_Model = load_model(TARGET_WEIGHTS_PATH)

x_=np.ones((8000,96,96,3))
y_=np.ones((8000,200))
m_=np.ones(8000)
i=0
classes=0
j=0
k=0
error_mat=[80,125,125,125,16,16,80,80,125,125,80,77,80,125,16,80,77,125,80,125,125,80,80,80,125,125,125,125,16,125,88,77,125,80,16,80,125,77,
125,125,16,77,80,77,125,125,80,16,125,77,125,125,125,125,80,77,125,16,80,80,125,77,125,125,125,125,16,125,32,16,125,125,16,125,125,
125,80,125,80,125,88,80,80,125,125,125,80,80,80,77,80,125,16,125,77,88,125,125,125,77,80,125,32,16,16,80,80,16,80,125,125,80,77,16,
80,16,77,80,16,77,125,125,77,125,80,77,16,16,125,80,125,16,125,77,125,125,16,125,125,80,16,125,125,125,16,77,125,77,125,125,32,80,
16,125,80,80,88,77,125,80,125,80,80,80,80,80,88,16,125,125,80,32,125,125,125,77,16,88,125,80,77,88,125,125,77,80,125,16,125,125,77,
80,16,16,80,125,16,80,88,125]
while i<7999:
    x_[i]=x_train[k]
    x_[i+1]=x_test[(error_mat[classes])*20+j]
    m_[i+1]=0
    y_[i]=y_train[k]
    y_[i+1]=y_test[(error_mat[classes])*20+j]
    i+=2
    k+=1
    j+=1
    if j%20==0:
...
def diff_Mem_attack(x_, y_true, m_true, target_model, non_Mem_Generator=sobel):
    ...
    dd = np.ones((8000, 5))
    for i in range(200):
        ll = np.c_[mix[i * 40:(i + 1) * 40][:], m_true[i * 40:(i + 1) * 40]]
        np.random.shuffle(ll)
        l = list(ll)
        dd[i * 40:(i + 1) * 40][:] = ll
    mix = tf.convert_to_tensor(dd[:, :4], dtype=float)
    m_true = dd[:, 4]
    data = tf.data.Dataset.from_tensor_slices((mix, m_true)).batch(20).prefetch(
        tf.data.experimental.AUTOTUNE)  # 切分mix为20000个10列的tensor，每个数据集有20个样本
    ...
    writer=pd.ExcelWriter('result_of_imagenet_DIFFCLASS.xlsx')
    ...
    writer = pd.ExcelWriter('C.xlsx')  # 写入Excel文件
...
m_true, m_pred, mix, nonMem = diff_Mem_attack(np.array(x_),y_,m_true,Target_Model)
