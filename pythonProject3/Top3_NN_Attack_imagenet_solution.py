...
x_train_tar=x_train_tar[:1200]
y_train_tar=y_train_tar[:1200]
labels=pd.read_excel('Top3_NN_incorrect_imagenet.xlsx')
...
m_test=m_test[:1200]
m_test=np.r_[m_test,np.zeros(1200)]

c_test = np.sort(Target_Model.predict(np.r_[x_train_tar, x_test_tar]), axis=1)[:, ::-1][:, :3]
