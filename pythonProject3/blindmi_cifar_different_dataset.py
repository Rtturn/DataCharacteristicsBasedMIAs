...
(x_train_tar, y_train_tar), (x_test_tar, y_test_tar), mmm= globals()['load_' + DATA_NAME]('TargetModel')
Target_Model = load_model(TARGET_WEIGHTS_PATH)

(x_c,y_c),(x_test_c,y_test_c),m_c=globals()['load_CIFAR10']('TargetModel')
#x_imagenet,y_imagenet,x_test_imagenet,y_test_imagenet,m_imagenet=globals()['load_imagenet']()
#(x_mnist,y_mnist),(x_test_mnist,y_test_mnist),m_mnist=globals()['load_CH_MNIST']('TargetModel')
x_=np.r_[x_train_tar, x_test_tar]
#x_=x_[:5000]
y_=np.r_[y_train_tar, y_test_tar]
#y_=y_[:5000]
m_=np.ones(20000)
#x_m_=np.r_[x_mnist, x_test_mnist]
#y_m_=np.r_[y_mnist, y_test_mnist]
x_c_=np.r_[x_c, x_test_c]
y_c_=np.r_[y_c, y_test_c]
m_m=np.zeros(20000)
x=np.r_[x_,x_c_]
y=np.r_[y_,y_c_]
m_true=np.r_[m_,m_m]
...
mix = tf.convert_to_tensor(np.c_[y_pred[y_.astype(bool)], np.sort(y_pred, axis=1)[:, ::-1][:, :2]] )
x_c_=np.r_[x_c, x_test_c]
y_c_=np.r_[y_c, y_test_c]
y_pred_c=Target_Model.predict(x_c_)
mix_c = tf.convert_to_tensor(np.c_[y_pred_c[y_c_.astype(bool)], np.sort(y_pred_c, axis=1)[:, ::-1][:, :2]] )
x_m_=np.r_[x_mnist, x_test_mnist]
y_m_=np.r_[y_mnist, y_test_mnist]
y_pred_m=Target_Model.predict(x_m_)
mix_m = tf.convert_to_tensor(np.c_[y_pred_m[y_m_.astype(bool)], np.sort(y_pred_m, axis=1)[:, ::-1][:, :2]] )
mixed=mix[:5000][:]
#mixed_c=mix_c[:5000][:]
x_i_=np.r_[x_imagenet, x_test_imagenet]
y_i_=np.r_[y_imagenet, y_test_imagenet]
y_pred_i=Target_Model.predict(x_i_)
mix_i = tf.convert_to_tensor(np.c_[y_pred_i[y_i_.astype(bool)], np.sort(y_pred_i, axis=1)[:, ::-1][:, :2]] )
mix_i=mix_i[80000:][:]
mixed_i=mix_i[:5000][:]
'''
'''
i=0
mmd=0
data = tf.data.Dataset.from_tensor_slices((mix, mix_c)).batch(20).prefetch(tf.data.experimental.AUTOTUNE)
for (m_batch,m_c_batch) in data:
    i+=1
    mmd+=mmd_loss(m_batch,m_c_batch,weight=1)
print(mmd/i,'mmd_cifar100_cifar10')
i=0
mmd=0
data = tf.data.Dataset.from_tensor_slices((mixed, mix_m)).batch(20).prefetch(tf.data.experimental.AUTOTUNE)
for (m_batch,m_c_batch) in data:
    i+=1
    mmd+=mmd_loss(m_batch,m_c_batch,weight=1)
print(mmd/i,'mmd_cifar100_mnist')
i=0
mmd=0
data = tf.data.Dataset.from_tensor_slices((mix, mix_i)).batch(20).prefetch(tf.data.experimental.AUTOTUNE)
for (m_batch,m_c_batch) in data:
    i+=1
    mmd+=mmd_loss(m_batch,m_c_batch,weight=1)
print(mmd/i,'mmd_cifar100_imagenet')
i=0
mmd=0
data = tf.data.Dataset.from_tensor_slices((mix_c, mix_i)).batch(20).prefetch(tf.data.experimental.AUTOTUNE)
for (m_batch,m_c_batch) in data:
    i+=1
    mmd+=mmd_loss(m_batch,m_c_batch,weight=1)
print(mmd/i,'mmd_cifar10_imagenet')

i=0
mmd=0
data = tf.data.Dataset.from_tensor_slices((mixed_c, mix_m)).batch(20).prefetch(tf.data.experimental.AUTOTUNE)
for (m_batch,m_c_batch) in data:
    i+=1
    mmd+=mmd_loss(m_batch,m_c_batch,weight=1)
print(mmd/i,'mmd_cifar10_mnist')

i=0
mmd=0
data = tf.data.Dataset.from_tensor_slices((mix_m, mixed_i)).batch(20).prefetch(tf.data.experimental.AUTOTUNE)
for (m_batch,m_c_batch) in data:
    i+=1
    mmd+=mmd_loss(m_batch,m_c_batch,weight=1)
print(mmd/i,'mmd_imagnet_mnist')
'''
def diff_Mem_attack(x_, y_true, m_true, target_model, non_Mem_Generator=sobel):
    ...
    for (mix_batch, m_true_batch) in data:  #min_batch:x  m_true_batch:是否为成员
        ...
        if len(nonmem_correct_dataset[0])>0 and len(nonmem_correct_dataset)>0 and len(nonmem_incorrect_dataset[0])>0 and len(nonmem_incorrect_dataset)>0:
    ...
    writer=pd.ExcelWriter('result_of_cifar100_cifar10.xlsx')
...
m_true, m_pred, mix, nonMem = diff_Mem_attack(x,y,m_true,Target_Model)
