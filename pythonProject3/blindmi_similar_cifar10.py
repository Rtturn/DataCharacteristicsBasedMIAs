...
def diff_Mem_attack(x_, y_true, m_true, target_model, non_Mem_Generator=sobel):
    ...
    m_true=dd[:,4]
    data = tf.data.Dataset.from_tensor_slices((mix, m_true)).batch(20).prefetch(tf.data.experimental.AUTOTUNE)  #切分mix为20000个10列的tensor，每个数据集有20个样本
    ...
    writer=pd.ExcelWriter('result_of_CIFAR10_simi.xlsx')
