...
j=0
k=0
error_mat=[3,0,3,0,3,3,3,0]
while i<4799:
    x_[i]=x_train_tar[k]
    x_[i+1]=x_test_tar[(error_mat[classes])*300+j]
    m_[i+1]=0
    y_[i]=y_train_tar[k]
    y_[i+1]=y_test_tar[(error_mat[classes])*300+j]
    i+=2
    k+=1
    j+=1
    if j%300==0:
...
def diff_Mem_attack(x_, y_true, m_true, target_model, non_Mem_Generator=sobel):
    ...
    dd = np.ones((4800, 5))
    for i in range(8):
        ll = np.c_[mix[i * 600:(i + 1) * 600][:], m_true[i * 600:(i + 1) * 600]]
    ...
    writer=pd.ExcelWriter('result_of_CH_MNIST_DIFFCLASS.xlsx')
